...
function getUrlVars(){var e={};window.location.href.replace(/[?&]+([^=&]+)=([^&]*)/gi,function(t,o,n){e[o]=n});return e}function CapCalCtrl(e,t,o,n){e.modsTaken=function(){var t={method:"GET",url:"index.php?id=modsTaken&token="+token+"&studentID="+sessionStorage.netid};o(t).then(function(t){e.modsCount=t.data.Results.length,e.takenMods=t.data.Results},function(e){console.log("error")})}}var token;null==sessionStorage.getItem("token")?(token=getUrlVars().token,sessionStorage.setItem("isValidToken",!1)):(token=sessionStorage.getItem("token"),sessionStorage.setItem("isValidToken",!0));var userName,studentId;angular.module("NUSAP").controller("CapCalCtrl",["$scope","$cookieStore","$http","$location",CapCalCtrl]);
